...
class YHSM_Cmd_AEAD_Validate_OTP(YHSM_AEAD_Cmd):
    ...
    def __init__(self, stick, public_id, otp, key_handle, aead):
        self.public_id = util.input_validate_nonce(public_id, pad = True)
        self.otp = util.input_validate_bytes(otp, 'otp', exact_len = defines.YSM_OTP_SIZE)
        self.key_handle = util.input_validate_key_handle(key_handle)
        aead = util.input_validate_aead(aead, expected_len = defines.YSM_YUBIKEY_AEAD_SIZE)
        # typedef struct {
        ...
        packed = struct.pack(fmt, self.public_id, \
                                 ...
                                 self.otp, \
                                 aead)
        YHSM_AEAD_Cmd.__init__(self, stick, defines.YSM_AEAD_YUBIKEY_OTP_DECODE, packed)
